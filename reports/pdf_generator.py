# ============================================================
# reports/pdf_generator.py — Generador de informes PDF
# Construye el payload del informe y genera el binario PDF
# completo usando ReportLab para el Dashboard Financiero.
# ============================================================
...
class GeneratedPdfReport:
    ...
    generated_at: str    # Timestamp de generación en ISO format
    warnings: list[str]  # Advertencias registradas durante la generación
    sections: list[str]  # Secciones incluidas en el informe


# ------------------------------------------------------------
# Verificación de disponibilidad de ReportLab
# ------------------------------------------------------------
...
def build_report_payload(
    *,
    ...
    Extrae y formatea datos de todos los snapshots del dashboard.
    """
    ...
    # Extraer resúmenes de cada snapshot
    ...
    # Consolidar y deduplicar advertencias de HRP y advisor
    warnings = [
    ...
    # Extraer tablas de datos de cada snapshot
    ...
    report_rows = {
        # Datos generales del usuario y resumen del portfolio
        "user_rows": [
        ...
        # Composición por activo (ticker, nombre, cantidad, valor, peso)
        "composition_rows": [
            [
                item.get("ticker", "n/d"),
        ...
        # Detalle de posiciones del portfolio
        "position_rows": [
            [
                item.get("portfolio_name", "n/d"),
                item.get("ticker", "n/d"),
        ...
        "evolution_rows": [
            ...
            for item in evolution_series[-12:]  # Solo los últimos 12 meses
        ],
        # Pesos actuales del portfolio ordenados de mayor a menor
        "current_weight_rows": [
            [
                item.get("ticker", "n/d"),
                item.get("asset_name", "n/d"),
            ...
            for item in sorted(
                weights_table,
                key=lambda row: float(row.get("current_weight", 0.0)),
                reverse=True
            )
        ],
        ...
        "hrp_weight_rows": [
            ...
            for item in sorted(
                weights_table,
                key=lambda row: float(row.get("recommended_weight", 0.0)),
                reverse=True
            )
        ],
        ...
        # Diagnóstico técnico del motor HRP
        "diagnostic_rows": [
    ...
    # Secciones que componen el informe PDF
    sections = [
        "Datos del usuario",
        "Portfolio",
        "Composición",
...
def generate_user_report_pdf(
    *,
    ...
    Usa ReportLab para construir el documento con estilos y tablas.
    """
    ...
    body_style = styles_module.ParagraphStyle(
        "ReportBody", parent=styles["BodyText"], leading=15, spaceAfter=8,
    )
    warning_style = styles_module.ParagraphStyle(
        "ReportWarning", parent=styles["BodyText"],
        textColor=colors.HexColor("#92400E"),
        backColor=colors.HexColor("#FEF3C7"),
        borderPadding=8, leading=14, spaceAfter=8,
    )
    footnote_style = styles_module.ParagraphStyle(
        "ReportFootnote", parent=styles["BodyText"],
        textColor=colors.HexColor("#475569"), fontSize=9, leading=12,
    )

    # Crear documento PDF en orientación horizontal A4
    buffer = BytesIO()
    ...
    # Construir el contenido del documento (story)
    story: list[Any] = []
    ...
    # Advertencias de datos (si las hay)
    if payload["warnings"]:
    ...
    # Sección: Posiciones y valor total
    ...
    # Nueva página para secciones de pesos y rebalanceo
    story.append(platypus.PageBreak())

    # Sección: Pesos actuales
    ...
    # Sección: Pesos HRP recomendados
    ...
    # Sección: Comentario final generado automáticamente
    ...
    # Construir el PDF con cabecera y pie de página en todas las páginas
    document.build(
        story,
        onFirstPage=_page_header_footer(modules),
        onLaterPages=_page_header_footer(modules),
    )
...
# ------------------------------------------------------------
# Construcción del comentario final automático
# ------------------------------------------------------------
...
def _build_final_comment(
    *,
    ...
    base_comment = (
        f"El portfolio consolida un valor estimado de {total_value}, "
        f"con una rentabilidad acumulada de {cumulative_return} "
        f"y anualizada de {annualized_return}. "
        f"El advisor identifica {increase_count} activos para aumentar, "
        f"{reduce_count} para reducir y {hold_count} para mantener."
    )

    # Agregar advertencias al comentario si las hay
    if warnings:
...
# ------------------------------------------------------------
# Construcción de tablas ReportLab
# ------------------------------------------------------------
...
def _build_table(
    ...
    Cabecera en fondo oscuro, filas alternadas en blanco y gris claro.
    """
    colors = modules["colors"]
    platypus = modules["platypus"]
    ...
    table.setStyle(platypus.TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0F172A")),   # Fondo cabecera oscuro
        ...
        ("BACKGROUND", (0, 1), (-1, -1), colors.HexColor("#F8FAFC")),  # Fondo filas claro
...
def _page_header_footer(modules: dict[str, Any]):
    """Devuelve el callback que dibuja cabecera y pie de página en cada hoja."""
    colors = modules["colors"]
    ...
    def _callback(canvas: Any, document: Any) -> None:
        ...
        # Texto izquierdo: nombre del proyecto
